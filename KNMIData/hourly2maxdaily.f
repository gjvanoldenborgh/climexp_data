...
!
     ...
     +       iname,stations(maxstations),nstations,years(2,maxstations)
        integer t,td,rh,tds(24),oldtds(24),rhs(24),maxpr,maxtp,maxtd
        real lon,lat,hgt
 ...
 100        continue
            ...
            if ( line(1:6).eq.'# STN,' ) then
                if ( line.ne.'# STN,YYYYMMDD,   HH,    T,   TD,   RH' )
     +               then
                    write(0,*) 'maybe wrong variables in file ',
     +                   trim(file),'?'
                    write(0,*) 'expected # STN,YYYYMMDD,   HH,    T,'//
     +                   '   TD,   RH'
            ...
            i = index(line,',',.true.)
            if ( line(i+1:) == ' ' ) goto 100
            read(line,*,err=901,end=901) station,datum,hr,t,td,rh
            if ( rh.eq.-1 ) rh = 0 ! round down 0<rh<0.05
            yr = datum/10000
            do istation = 1,nstations
                if ( station == stations(istation) ) exit
            end do
     ...
     +               'dew point temperature'
            ...
            if ( datum /= olddatum ) then
                if ( olddatum /= -1 ) then
                    ...
                    maxtd = -9999
                    do i=1,24
                        if ( tds(i) > maxtd .and. tds(i).lt.9999 ) then
                            maxtd = tds(i)
                        end if
                        if ( rhs(i) > maxpr .and. rhs(i).lt.9999 ) then
                            maxpr = rhs(i)
                            if ( i > 4 ) then
                                maxtp = tds(i-4)
                            else
                    ...
                    if ( maxpr.ge.0 .and. maxpr.lt.9999 ) then
                        write(10+3*istation,'(i8,f7.1)')
     +                       olddatum,maxpr/10.
                    end if
                    if ( maxtp.ge.0 .and. maxtp.lt.9999 ) then
                        write(11+3*istation,'(i8,f7.1)')
     +                       olddatum,maxtp/10.
                    end if
                    if ( maxtd.ge.0 .and. maxtd.lt.9999 ) then
...
!           next file
 ...
 801        continue
            ...
            if ( line(1:6).eq.'# STN ' ) then
                ...
                if ( j.eq.stations(istation) ) then
                    if ( j.eq.215 ) then
                        lat = -999.9
                        lon = -999.9
                        hgt = -999.9
