...
!   make the Central Netherlands Temperature
    ...
    if ( string == ' ' .or. string == '1.0' .or. string == '1' ) &
    then
        file = 'CNT4_6.dat'
    ...
    do yr=2000,yrend
        do mo=1,12
            ...
            n = 0
            do i=1,nfiles
                if ( series(mo,yr,i) < 1e33 ) then
                    s = s + series(mo,yr,i)
                    n = n + 1
                end if
            end do
            s = s/n
            if ( cnt(mo,yr) < 1e33 ) then
            !   check
                if ( abs(cnt(mo,yr) - s) > 0.06 ) then
