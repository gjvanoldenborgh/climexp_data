...
C****
      read(*,*) taskf,typef,IYR1f,IYR2f, IY1bf,IY2bf, ! =MNYRSGf,MINY1Gf
     *          rlnd,imo,jmo,offio,divjo
      iskip=0
      if(taskf.le.0) then
        iskip=abs(taskf)
        series=.true.
...
!!!   if(divjo.lt.float(jmo)-.9) ipol=1
!!!   if(divjo.gt.float(jmo)+.1.and.divjo.lt.float(jmo)+.9) ipol=1
      if(divjo.lt.0.) then           ! force polar averaging
        divjo=-divjo
        ipol = 1
      end if
...
C**** End of loop over subboxes - Initiate interpolation to model grid
      call eantrp0 (imo,jmo,offio,divjo,BAD)
C****
...
C****
  ...
  200 CALL SWRITE(11,TITLE,TGCM,imo,jmo,bad,nmaps,divjo,offio)
      STOP
      END
