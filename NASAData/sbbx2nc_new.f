...
!!!      parameter (iocn = 0)      ! 0 no ocean, 1 ocean only, 2 land+ocean
      ...
      parameter (ifyr = 1880, ilyr = 2020)   ! first, last years of output
...
C
        allocate(tout(monmc*im*jm))
      do 5100 I = 1, monmc*im*jm
        tout(I) = bad
 5100 continue
C
...
C
      ...
      is = nf_def_DIM(ncid, 'time', monmc, timedim)
      if (is .ne. nf_noerr) call handle_err(is)
...
C
          if (tav(1).ne.bad) then
             tav(1) = tav(1) - tavb
             call embed (tav, tout, im, jm, monmc,
