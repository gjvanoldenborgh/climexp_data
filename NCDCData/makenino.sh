...
do
  ...
  normdiff ersst_nino${nino}.dat ersst_tropical.dat none none > ersst_nino${nino}_detrend.dat
  egrep '^#' ersst_nino${nino}.dat | sed -e 's/SST/SSTA/' > ersst_nino${nino}a_detrend.dat
  echo '# SSTA normalized to 1880-now' >> ersst_nino${nino}a_detrend.dat
  plotdat anomal 1981 2010 ersst_nino${nino}_detrend.dat | fgrep -v repeat | fgrep -v normal  >> ersst_nino${nino}a_detrend.dat
done
