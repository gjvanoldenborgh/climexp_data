...
*
        implicit none
#include "recfac.h"
        integer i,j,k,iyear,imon,n,ldataset,ii,idata(72,36),yr,mo
     +       ,missing,yrbeg,yrend
        real data(72,36)
        character dataset*255,var*4,lvar*40,units*10,format(2)*40,
     +       version*10
...
*       
        ...
        if ( index(dataset,'blended').ne.0 .or.
     +       index(dataset,'merged').ne.0 ) then
            var = 't'
            ...
            missing = -9999
            yrbeg=1880
        else if ( index(dataset,'prcp').eq.0 ) then
            var = 'temp'
     ...
     +       ='direct',recl=72*36*recfa4)
        ...
        do yr=yrbeg,yrend
            do mo=1,12
                ...
                !!!print *,iyear,imon
                if ( iyear.ne.yr .or. imon.ne.mo ) then
                ...
                do j=1,36
                    do i=1,72
                        if ( idata(i,37-j).eq.missing ) then
                            data(i,j)= 3e33
                        else
