...
*
     ...
     +        ,ifac,nmin,rmin,elevmin,elevmax,list,nl,nlist)
        ...
        integer n,nn,istation,ifac,nmin(0:48),nl,list(nl),nlist,narg
*       
...
*
     ...
     +                ,sname(1:llen(sname))
        ...
        else if ( string(1:4).eq.'list' ) then
...
*
        ...
        parameter (nrecslv=44881,nstatslv=999103)
        logical lwrite
...
*  #[ readstation:
   ...
   10 continue
      ...
      if ( ifac.eq.1 ) then
          ...
          k = j + 1
          do while ( ichar(string(k:k)).ge.ichar('0')
     +         .and. ichar(string(k:k)).le.ichar('9') )
              k = k + 1
          end do
          read(string(j:k-1),*,err=904) fstation
