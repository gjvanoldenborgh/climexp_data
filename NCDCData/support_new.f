...
*
     ...
     +        ,ifac,nmin,rmin)
        implicit none
        character sname*(*)
        real slat,slon,slat1,slon1,rmin
        integer n,nn,istation,ifac,nmin(0:48)
*       
...
*
     ...
     +            ichar(string(1:1)).le.ichar('9') ) then
                if ( ifac.eq.1 ) then
                    read(string,*) istation
                else
                    read(string,*,err=904) fstation
                    istation = nint(ifac*fstation)
                    i = 4                
                    if ( string(i:i).eq.'.' ) i = 5
                    j = ichar(string(i:i))
                    if ( i.ge.ichar('a') .and. j.le.ichar('z') ) then
                        istation = istation + j - ichar('a')
                    elseif ( i.ge.ichar('A') .and. j.le.ichar('Z') )
     +                        then
                        istation = istation + j - ichar('A')
                    endif
                endif
...
*
*       old interface - still used by many get* programs
*
        implicit none
        character*(*) type
        integer iu,ii,n,nyr(0:48)
        integer firstrec,lastrec
        call getdataindex(type,iu,ii,n,nyr,firstrec,lastrec)
        end
        subroutine getdataindex(type,iu,ii,n,nyr,firstrec,lastrec)
*
*       print precip data - use binary search to find the records in
*       the (sorted) direct access file v2.precip.beta.data open on unit
*       iu.  Also returns the first and last records, so future access
*       can be quicker.
*
        implicit none
        character*(*) type
        integer iu,ii,n,nyr(0:48),firstrec,lastrec
        integer j,k,kk,l,ilo,ihi,imi,i0,yr,yrm1,yr1,yr2,prcp(24),md
*       should move these to the calling programs one day...
        integer nrecprcp,nstatprcp
        parameter (nrecprcp=1149062,nstatprcp=38169)
        integer nrectemp,nstattemp
        parameter (nrectemp=293536,nstattemp=711890690)
        integer nrectmin,nstattmin
        parameter (nrectmin=195258,nstattmin=650336310)
        integer nrectmax,nstattmax
        parameter (nrectmax=193939,nstattmax=650336310)
        ...
        parameter (nrecslv=35482,nstatslv=999024)
        logical lwrite
...
*           found a match - scan it
            ...
            if ( n.eq.1 ) then
                if ( type.eq.'prcp' ) then
                    print '(a)'
     +                    ,'prcp from v2.precip.beta.data in mm/month'
                elseif ( type.eq.'temp' ) then
                    print '(2a)','temp from v2.mean_adj_nodup in ',
     +                    'degree celsius'
                elseif ( type.eq.'tmin' ) then
                    print '(2a)','tmin from v2.min_adj_nodup in ',
     +                    'degree celsius'
                elseif ( type.eq.'tmax' ) then
                    print '(2a)','tmax from v2.max_adj_nodup in ',
     +                    'degree celsius'
                elseif ( type.eq.'sea' ) then
                    print '(2a)','sealevelpressure from ',
     +                    'press.sea.data in mb'
                elseif ( type.eq.'sta' ) then
                    print '(2a)','stationpressure from ',
     +                    'press.sta.data in mb'
                elseif ( type.eq.'slv' ) then
...
*
        c5 = country
        if ( llen(c5).gt.3 ) then
            i = index(name(3:),c5(1:llen(c5))) ! dont kill SINGAPORE...
            if ( i.gt.0 ) name(i+2:) = ' '
        endif
*       a few spacial cases
        i = index(name,'   ')
