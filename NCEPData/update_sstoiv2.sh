...
for file in sstoi_v2.ctl
do
    case $file in
	sstoi_v2.ctl) var=sst;outfile=nonmissing_`basename $file .ctl`.nc;;
...
quit
EOF
	cdo -b 32 -r settaxis,1981-11-01,0:00,1mon $outfile aap.nc
	mv aap.nc $outfile
    fi
done
...
do
    case $var in
	sst) file=sstoi_v2.ctl
	    lsmask=ls_sstoi_v2.nc
	...
	*) echo error udwiogfewjh; exit -1;;
	...
	series=sstoiv2_${var}_$area.dat
