...
while [ -s $file ]; do
    ...
    wget -q -N ftp://ftp.cdc.noaa.gov/Datasets/noaa.oisst.v2.highres/sst.day.anom.$i.v2.nc
    ...
    if [ ! -s nino12_daily_$i.dat -o nino12_day_$i.dat -ot sst.day.anom.$i.v2.nc ]; then
        get_index sst.day.anom.$i.v2.nc 270 280 -10 0 > nino12_daily_$i.dat
    fi
    if [ ! -s nino3_daily_$i.dat -o nino3_day_$i.dat -ot sst.day.anom.$i.v2.nc ]; then
        get_index sst.day.anom.$i.v2.nc 210 270 -5 5 > nino3_daily_$i.dat
    fi
    if [ ! -s nino34_daily_$i.dat -o nino34_day_$i.dat -ot sst.day.anom.$i.v2.nc ]; then
        get_index sst.day.anom.$i.v2.nc 190 240 -5 5 > nino34_daily_$i.dat
    fi
    if [ ! -s nino4_daily_$i.dat -o nino4_day_$i.dat -ot sst.day.anom.$i.v2.nc ]; then
