...
!
    ...
    integer yr,mo,dy,isn,i,k,n,nsn(-1:1),yr1,mo1,dy1,dpm(12,2),idata(max(316*332,304*448))
...
!
 ...
 &                       'nt_',yr,mo,'_f',k,'_pre_',csn(isn),'.bin'
                ...
                if ( lexist .or. yr1.lt.0 ) then
                    if ( lexist .and. lwrite ) print *,'found monthly file ',yr,mo
                    cycle
                endif
!
...
!
                ...
                do dy=1,dpm(mo,leap(yr))
                    if ( lwrite ) print *,'searching for daily file ',yr,mo,dy
                    do k=20,1,-1
                        write(file,'(a,i4,2i2.2,a,i2.2,a,a,a)') &
 &                           'nt_',yr,mo,dy,'_f',k,'_nrt_',csn(isn),'.bin'
                        inquire(file=file,exist=lexist)
                        if ( lexist ) exit
                    end do  ! k
                    if ( lexist ) then
                        if ( lwrite ) print *,'found daily file'
                        open(10+dy,file=file,access='direct',recl=recfa4/4)
                        dy1 = dy
                    else
                    ...
                    do i=1,nsn(isn)
                        ...
                        do dy=1,dpm(mo,leap(yr))
                            if ( ichar(cdata(i,dy)).le.250 ) then
                                n = n + 1
                                idata(i) = idata(i) + ichar(cdata(i,dy))
                            end if
                        end do ! dy
 ...
 &                       'nt_',yr,mo,'_f',k,'_nrt_',csn(isn),'.bin'
                    ...
                    write(0,*) 'cannot handle partial month',yr,mo,dy1,' yet'
                    goto 800
                end if
            end do          ! mo
        end do              ! yr
