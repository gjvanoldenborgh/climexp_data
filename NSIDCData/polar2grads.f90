...
!   to Grads latlon files
    ...
    real :: conxy(316,448),conll(360,latmin:89),iconll(360,84:89)
    real :: x,y,alat,alon,sgn,slat,e,re,e2,s,t,ss,tt,d
    character file*255,ifile*255,csn(-1:1),cval,infile*255,months(12)*3
...
!   set up grid conversion arrays
    ...
    do isn=-1,+1,2              ! -1: south, +1: north
        ...
        elseif ( isn == +1 ) then
            ...
            write(17,'(a)') '# estimated sea ice concentrations in 87-90N'
        else
            call exit(-1)
        endif
        sgn = isn
...
!               estimate ice concentration in data hole 87-90 N
                ...
                if ( isn == +1 ) then
                    if ( yr < 1987 .or. yr == 1987 .and. mo < 9 ) then
                        jhole = 84
                    else
                        jhole = 87
                    end if
                    ...
                    do j=jhole,89
                        do i=1,360
                            if ( conll(i,j) >= 1e30 ) then
                                s = 0
                                t = 0
                                do jj=jhole-2,jhole
                                    do ii=1,360
                                        if ( conll(ii,jj) < 1e33 ) then
                                            d = dist(i+0.5,j+0.5,ii+0.5,jj+0.5)
                                            s = s + conll(ii,jj)/d**2
                                            t = t + 1/d**2
                                        end if
                                    end do
                            ...
                            if ( iconll(i,j) < 1e30 ) then
                                ss = ss + iconll(i,j)*cos((jj+0.5)/180*4*atan(1.))
                                tt = tt + cos((jj+0.5)/180*4*atan(1.))
                            end if
                        end do
                    end do
                    do j=jhole,89
                    ...
                    write(11,rec=irec) ((conll(i,j),i=1,360),j=latmin,89)
                end if
            end do          ! mo
        end do              ! yr
    800 continue
        ...
        do i=2,ii,10
            ...
            if ( isn == +1 ) then
                ...
                write(i,'(a,i2,a,f5.1,a)') 'YDEF ',90-latmin,' LINEAR ',latmin+0.5,' 1'
            else
                write(i,'(a)') 'XDEF 360 LINEAR 0.5 1'
                write(i,'(a,i2,a,f5.1,a)') 'YDEF ',90-latmin,' LINEAR ',-89.5,' 1'
            endif
            write(i,'(a)') 'ZDEF 1 LINEAR 0 1'
            write(i,'(a,i4,2a,i4,a)') 'TDEF ',irec,' LINEAR 15',months(mo1),yr1,' 1MO'
            write(i,'(a)') 'VARS 1'
...
!   http://en.wikipedia.org/wiki/Great-circle_distance
    ...
    lon2 = x2*deg2rad
    lat2 = y2*deg2rad
    dist = atan2( sqrt( (cos(lat2)*sin((lon2-lon1)))**2 &
        + (cos(lat1)*sin(lat2)- sin(lat1)*cos(lat2)*cos(lon2-lon1))**2 ) &
