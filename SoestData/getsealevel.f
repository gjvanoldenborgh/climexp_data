...
*
	...
	parameter (pi  = 3.1415926535897932384626433832795d0)
        integer i,j,k,jj,kk,n,m,ldir,istation,nyr,nmin,yr,idat(2,6)
     +        ,ilev(12),nok
        integer iwmo(nn),imod(nn),ielevs(nn),firstyear(nn)
     +        ,lastyear(nn),ilat(2),ilon(2),ci(nn),ind(nn)
        real rlat(nn),rlon(nn),slat,slon,slat1,slon1,dist(nn)
     +        ,fstation,dlon,rmin
        character name(nn)*18,country(nn)*18
        character string*80,sname*18,version*1,ns*1,ew*1,v*1,nam*4
        character dir*256
        integer iargc,llen
...
*
        if ( istation.gt.0 ) then
            print '(a,i3.3,2a)',
     +            'Searching for JASL tide-gauge (sealevel) station nr '
...
*       check that we have enough years of data
        if ( nmin.gt.0 ) then
            if ( nyr*ci(i)/100..lt.nmin ) goto 100
***            print *,'OK'
        endif
*
        if ( istation.eq.0 ) then
...
*       output
     ...
     +            ,'=============================================='
            ...
            enddo
            print '(2a,a,a)',name(jj),'(',country(jj)(1:llen(country(jj)
     +            )),')' 
